...
const FREE_ITEMS = [
  { icon: "layers" as const, label: "Posters & QR Code", sub: "Print-ready materials to promote your plans in-store" },
  { icon: "scan" as const, label: "Mobile App for Customers", sub: "iOS & Android app so members can track perks and billing" },
  { icon: "bar-chart" as const, label: "Merchant Dashboard", sub: "Track members, revenue, and plan performance in one place" },
  { icon: "repeat" as const, label: "Automated Billing & Payout", sub: "Recurring charges and payouts handled without lifting a finger" },
];
...
const tiers = [
  {
    ...
    bullets: ["Unlimited members", "Multiple plan tiers & perks", "Stamp cards & rewards", "Cohort analytics", "Email & push"],
  },
  {
    pct: "Custom",
    ...
    bullets: ["Everything included", "Multiple locations", "API & webhooks", "Priority support", "Custom branding"],
  },
];

const explainer = [
  { icon: "users" as const, label: "Members pay you", value: "₱899/mo each", iconBg: "var(--teal-100)", iconColor: "var(--teal)" },
  { icon: "percent" as const, label: "Memberry takes", value: "8% commission", iconBg: "var(--ink)", iconColor: "#fff" },
  { icon: "wallet" as const, label: "You keep", value: "₱827.08/member", iconBg: "var(--gold-100)", iconColor: "var(--gold-700)" },
];

export default function DealSection() {
  return (
    <section id="pricing" className="py-16 px-5 md:py-24 md:px-8 bg-[var(--bg-app)] border-t border-[var(--border)]">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="text-center mb-14">
            ...
            <p className="text-[17px] text-[var(--fg-2)] font-[family-name:var(--font-body)] max-w-[520px] mx-auto m-0">
              No monthly fees. No setup costs. Just a small percentage of what your members pay you. We win when you win.
            </p>
          </motion.div>
          ...
          >
            ...
            <div className="relative z-10">
                  ...
                  <React.Fragment key={item.label}>
                    <div className="flex flex-col items-center gap-[10px] text-center">
                      <div
                        ...
                        style={{ borderRadius: 14, background: item.iconBg, color: item.iconColor }}
                      >
              ...
              <p className="text-center text-[13px] text-[var(--fg-3)] font-[family-name:var(--font-body)] mt-6 mb-0 border-t border-[var(--border)] pt-5">
                Example: 100 members at ₱899/mo = ₱89,900 revenue · ₱7,192 to Memberry · ₱82,708 in your pocket
              </p>
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} className="mb-8">
            <div className="bg-white border border-[var(--border)] rounded-xl p-6 md:p-8 max-w-[860px] mx-auto">
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="text-[12px] font-semibold text-[var(--fg-3)] font-[family-name:var(--font-body)]">— no setup fee, no investment required</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {FREE_ITEMS.map((item) => (
                  <div key={item.label} className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-[family-name:var(--font-body)] font-semibold text-[14px] text-[var(--ink)]">{item.label}</div>
                      <div className="font-[family-name:var(--font-body)] text-[13px] text-[var(--fg-3)] mt-0.5">{item.sub}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              >
                ...
                {tier.dark && (
                  <div
                    ...
                    style={{ backgroundImage: 'url("/assets/textures/dots.svg")' }}
                  />
                )}
                <div className="relative flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <span
                      ...
                      style={{ color: tier.dark ? "var(--teal-300)" : "var(--fg-2)" }}
                    >
                      {tier.label}
                    </span>
                    <span
                      className="text-[13px] font-[family-name:var(--font-body)] block mt-1.5"
                      style={{ color: tier.dark ? "rgba(255,255,255,0.5)" : "var(--fg-3)" }}
                    >
                      {tier.note}
                    </span>
                  </div>
                  ...
                  <div
                    className="my-1"
                    style={{ borderTop: `1px solid ${tier.dark ? "rgba(255,255,255,0.12)" : "var(--border)"}` }}
                  />
                  {/* Feature bullets */}
                    ...
                    {tier.bullets.map((b) => (
                      ...
                      >
                        <span
                          ...
                          style={{
                            background: tier.dark ? "var(--gold-100)" : "var(--teal-100)",
                            color: tier.dark ? "var(--gold-700)" : "var(--teal)",
                          }}
                        >
          ...
          >
            Volume discounts and custom contracts available for 500+ member programs.{" "}
            <a href="#contact" className="text-[var(--teal)] font-semibold no-underline">
              Talk to us →
            </a>
          </motion.p>
        </motion.div>
